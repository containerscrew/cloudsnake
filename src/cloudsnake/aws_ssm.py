...
from aws_client import ssm_client
...
class SSMSession:
    ...
    def _start_session_response(self):
    ...
    def start_session(self):
        ...
        try:
            ...
            if ex.errno == errno.ENOENT:
                self.logger.debug('SessionManagerPlugin is not present',
                             exc_info=True)
                print('SessionManagerPlugin is not present')
                self.client.terminate_session(SessionId=self.start_session_response['SessionId'])
