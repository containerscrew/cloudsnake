...
def generate_db_auth_token(
    ctx: typer.Context,
    hostname: str = typer.Option(..., "--hostname", "-h", help="Hostname of the RDS instance"),
    port: Optional[int] = typer.Option(3306, "--port", "-p", help="DB instance port"),
    username: str = typer.Option(..., "--username", "-u", help="Database username to connect", case_sensitive=True),
    print: Optional[bool] = typer.Option(True, "--no-print", "-np", help="Do not print the token in the console"),
):
    """Invoke RDS generate-db-auth-token"""
    rds = RDSInstanceConnectWrapper.with_client(
        "rds", ctx.obj.session, hostname=hostname, port=port, db_username=username, region=ctx.obj.region)
